package objects;

public class RandomNumbers {
    private int randomNum1;
    private int randomNum2;
    private int randomNum3;
    private int randomNum4;
    private int randomNum5;
    private int randomCount;
    private int randomShift;
    private int randomStart;

    public RandomNumbers(int randomNum1, int randomNum2, int randomNum3, int randomNum4, int randomNum5, int randomCount, int randomShift, int randomStart) {
        this.randomNum1 = randomNum1;
        this.randomNum2 = randomNum2;
        this.randomNum3 = randomNum3;
        this.randomNum4 = randomNum4;
        this.randomNum5 = randomNum5;
        this.randomCount = randomCount;
        this.randomShift = randomShift;
        this.randomStart = randomStart;
    }

    public RandomNumbers() {
    }

    public int getRandomNum1() {
        return randomNum1;
    }

    public void setRandomNum1(int randomNum1) {
        this.randomNum1 = randomNum1;
    }

    public int getRandomNum2() {
        return randomNum2;
    }

    public void setRandomNum2(int randomNum2) {
        this.randomNum2 = randomNum2;
    }

    public int getRandomNum3() {
        return randomNum3;
    }

    public void setRandomNum3(int randomNum3) {
        this.randomNum3 = randomNum3;
    }

    public int getRandomNum4() {
        return randomNum4;
    }

    public void setRandomNum4(int randomNum4) {
        this.randomNum4 = randomNum4;
    }

    public int getRandomNum5() {
        return randomNum5;
    }

    public void setRandomNum5(int randomNum5) {
        this.randomNum5 = randomNum5;
    }

    public int getRandomCount() {
        return randomCount;
    }

    public void setRandomCount(int randomCount) {
        this.randomCount = randomCount;
    }

    public int getRandomShift() {
        return randomShift;
    }

    public void setRandomShift(int randomShift) {
        this.randomShift = randomShift;
    }

    public int getRandomStart() {
        return randomStart;
    }

    public void setRandomStart(int randomStart) {
        this.randomStart = randomStart;
    }

    @Override
    public String toString() {
        return "RandomNumbers{" +
                "randomNum1=" + randomNum1 +
                ", randomNum2=" + randomNum2 +
                ", randomNum3=" + randomNum3 +
                ", randomNum4=" + randomNum4 +
                ", randomNum5=" + randomNum5 +
                ", randomCount=" + randomCount +
                ", randomShift=" + randomShift +
                ", randomStart=" + randomStart +
                '}';
    }
}
